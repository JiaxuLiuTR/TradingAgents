...
def _resolve_save_report(save: bool | None, *, non_interactive: bool) -> bool | None:
    """Resolve final report saving.

    ``None`` means "ask the user" in interactive mode. Unattended runs cannot
    prompt, so omission keeps the documented auto-save default.
    """
    if save is not None:
        return save
    if non_interactive:
...
def run_analysis(
    ...
    # non-interactive mode the report defaults to auto-save and the on-screen
    # display is skipped, so the run never blocks on input.
    console.print("\n[bold cyan]Analysis Complete![/bold cyan]\n")
    console.print(f"[dim]{analyst_wall_time_tracker.format_summary()}[/dim]")

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    default_path = Path.cwd() / "reports" / f"{selections['ticker']}_{timestamp}"
    save_report = _resolve_save_report(save, non_interactive=non_interactive)
    ...
    if save_report is False:
        console.print("[yellow]Report save skipped.[/yellow]")
        if non_interactive:
            return
    ...
    if save_report is None and typer.prompt("Save report?", default="Y").strip().upper() in (
        "Y",
        "YES",
        "",
    ):
...
def analyze(
    ...
    save: bool | None = typer.Option(
        None,
        "--save/--no-save",
        help="Save or skip the final consolidated report. Omit to auto-save in "
        "unattended --ticker runs, or to ask in interactive runs.",
    ),
